...
def ObjectFucntion(selection,tc,details_testsuit,affected_ppi,affected_pp):
    ...
    return math.pow(riskScore,2)+10*costScore/float(total_execution);
...
budgetSet = [1327,2655,3981,5308,6635]
...
if __name__ == '__main__':
    ...
    for i in range(len(budgetSet)):
        ...
        # case 1
        #setAffectedITM(["summary"],["jot-summary","summaryidea"])
        # case 2
        setAffectedITM(["thread.prepare", "thread.init", "thread.noteRead", "thread.createScaffoldStartTag",
                        "thread.get_project_info"], ["note_str_id", "threadid", "node_id", "thread_id"])
